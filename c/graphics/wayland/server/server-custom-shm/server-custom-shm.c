...
{
    fprintf(stderr, "shm_buffer_client_destroy_notify\n");
	struct wl_shm_buffer *buffer = wl_container_of(listener, buffer, client_destroy_listener);
	buffer->client = NULL;
...
static void shm_pool_create_buffer(
	...
	buffer->client_destroy_listener.notify = shm_buffer_client_destroy_notify;
	wl_client_add_destroy_listener(buffer->client, &buffer->client_destroy_listener);
    ...
    wl_resource_set_implementation(buffer->resource, &buffer_impl, buffer, destroy_buffer);
...
static void shm_create_pool(
	...
	pool->resource = wl_resource_create(client, &wl_shm_pool_interface, version, id);
    wl_resource_set_implementation(pool->resource, &shm_pool_impl, pool, destroy_pool);
...
static void surface_attach(
    struct wl_client *client,
    // The actual buffer is resource->data
    struct wl_resource *resource,
    ...
    // surface_buffer = buffer->data
    surface_buffer = wl_shm_buffer_get(buffer);

    if (!surface_buffer) {
        fprintf(stderr, "surface_buffer is null\n");
    }
...
int main(void) {
    ...
    wl_global_create(wl_display, &wl_shm_interface, 2, NULL, bind_shm);
