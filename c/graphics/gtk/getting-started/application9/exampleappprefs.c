...
static gboolean string_to_font_desc(GValue* value, GVariant* variant, gpointer user_data) {
...
static GVariant* font_desc_to_string(const GValue* value, const GVariantType* expected_type,
                                     gpointer user_data) {
...
static gboolean transition_to_pos(GValue* value, GVariant* variant, gpointer user_data) {
  const char* s = g_variant_get_string(variant, NULL);
  if (strcmp(s, "none") == 0)
...
static GVariant* pos_to_transition(const GValue* value, const GVariantType* expected_type,
                                   gpointer user_data) {
...
static void example_app_prefs_init(ExampleAppPrefs* prefs) {
  ...
  g_settings_bind_with_mapping(prefs->settings, "font", prefs->font, "font-desc",
                               G_SETTINGS_BIND_DEFAULT, string_to_font_desc, font_desc_to_string,
                               NULL, NULL);
  g_settings_bind_with_mapping(prefs->settings, "transition", prefs->transition, "selected",
                               G_SETTINGS_BIND_DEFAULT, transition_to_pos, pos_to_transition, NULL,
                               NULL);
}
