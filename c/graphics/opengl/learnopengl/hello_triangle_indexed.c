...
int main() {
    ...
    // Specify which vertices to draw and in which order
    GLuint ebo;
    glGenBuffers(1, &ebo);
    ...
    // It's possible to unbind VBO, the call to glVertexAttribPointer registered 
    // VBO as the vertex attribute's bound vertex buffer object so afterwards
    // we can safely unbind. Also VAO can be unbound. These unbindings are done
    // so other VAO and VBO calls won't accidentally modify these VBO and VAO, 
    // but this rarely happens. Modifying other VAOs requires a call to
    // glBindVertexArray anyways so we generally don't unbind VAOs (nor VBOs)
    // when it's not directly necessary. The ebo, on the other hand, should be
    // kept bound.
